...
class InvestorMetricsHousingEventCounts(ParclLabsService):
    ...
    def retrieve(
        ...
        results =  self._request(
            url=f'/v1/investor_metrics/{parcl_id}/housing_event_counts', 
            params=params
        )
...
class InvestorMetricsPurchaseToSaleRatio(ParclLabsService):
    ...
    def retrieve(
        ...
        results =  self._request(
            url=f'/v1/investor_metrics/{parcl_id}/purchase_to_sale_ratio', 
            params=params
        )
    ...
    def _as_pd_dataframe(
        ...
        for k, l in data.items():
            for v in l:
                ...
                names = ['rolling_7_day', 'rolling_30_day', 'rolling_60_day', 'rolling_90_day']
                tmp = pd.DataFrame({'date': date, 'period': names, 'counts': counts, 'pct_for_sale_market': pcts})
                tmp['parcl_id'] = k
                out.append(tmp)
