...
@click.option(
    "--market",
    required=True,
    help='The market to search. Use "United States" for nationwide searches.',
)
@click.option(
    "--current-entity-owner-name",
    default=None,
    help="Filter by the current entity owner name.",
)
@click.option(
    "--event-type",
    required=True,
    type=click.Choice(["sale", "rental", "listing"], case_sensitive=False),
    help="The type of event to retrieve.",
)
@click.option(
    "--output-file", required=True, help="The name and location of the output file."
)
@click.option(
    "--start-date", default="2024-07-24", help="The start date for the event search."
)
@click.option(
    "--end-date", default="2024-08-25", help="The end date for the event search."
)
def main(
    market, current_entity_owner_name, event_type, output_file, start_date, end_date
):
    ...
    click.echo(
        f"Market data found: Name: {name}, Location Type: {location_type}, Total Population: {total_population:,}, Median Income: ${income:,}"
    )
    ...
    entity_name = current_entity_owner_name.replace(
        "invitation_homes", "Invitation Homes"
    ).replace("amh", "American Homes 4 Rent")

    if event_type == "rental":
        click.echo(
            f"Summary {entity_name}: {distinct_units} units on {event_type} market in {market} with a median price of ${median_price:,.2f} an estimated vacancy rate of {distinct_units/total_units:.2%}"
        )
        ...
        click.echo(
            f"Summary: {distinct_units} units on {event_type} market in {market}"
        )
