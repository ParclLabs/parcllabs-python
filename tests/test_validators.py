...
from parcllabs.services.validators import Validators  # replace 'your_module' with the actual module name
...
def test_validate_property_type():
    ...
    assert str(excinfo.value) == f"Property type VILLA is not valid. Must be one of {', '.join(VALID_PROPERTY_TYPES)}."
...
def test_validate_portfolio_size():
    ...
    assert str(excinfo.value) == f"Portfolio size EXTRA-LARGE is not valid. Must be one of {', '.join(VALID_PORTFOLIO_SIZES)}."
