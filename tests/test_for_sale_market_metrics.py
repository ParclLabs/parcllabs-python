...
from parcllabs.services.for_sale_market_metrics import ForSaleMarketMetricsNewListingsRollingCounts
...
def test_retrieve_success(service, mock_client, api_response):
    ...
    result = service.retrieve(parcl_id=101, start_date="2023-01-01", end_date="2023-01-08", property_type="condo")
...
def test_retrieve_as_dataframe(service, mock_client, api_response):
    mock_client.get.return_value = api_response
    result = service.retrieve(parcl_id=101, start_date="2023-01-01", end_date="2023-01-08", property_type="condo", as_dataframe=True)
...
def test_retrieve_many_success(service, mock_client, api_response):
    mock_client.get.return_value = api_response
    parcl_ids = [101, 102]
    result = service.retrieve_many(parcl_ids=parcl_ids, start_date="2023-01-01", end_date="2023-01-08", property_type="condo")
...
def test_retrieve_many_as_dataframe(service, mock_client, api_response):
    mock_client.get.return_value = api_response
    parcl_ids = [101, 102]
    result = service.retrieve_many(parcl_ids=parcl_ids, start_date="2023-01-01", end_date="2023-01-08", property_type="condo", as_dataframe=True)
    assert isinstance(result, pd.DataFrame)
    assert 'parcl_id' in result.columns
...
def test_retrieve_api_error(service, mock_client):
    ...
    with pytest.raises(HTTPError):
        service.retrieve(parcl_id=101, start_date="2023-01-01", end_date="2023-01-08", property_type="condo")
