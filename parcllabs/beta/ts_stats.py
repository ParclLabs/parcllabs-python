...
class TimeSeriesAnalysis:
    ...
    def calculate_52_week_high_low(self):
        ...
        high_date = last_year_df[last_year_df[self.value_col] == high_value][self.date_col].iloc[0].date().isoformat()
        low_date = last_year_df[last_year_df[self.value_col] == low_value][self.date_col].iloc[0].date().isoformat()
        return {
    ...
    def calculate_changes(self, change_since_date=None):
        ...
        for key, offset in intervals.items():
            ...
            if not past_df.empty:
                ...
                results[key] = {
                    'value_diff': round(float(value_diff), 2),  # Convert to native Python float
        ...
        results['peak_to_current'] = {
            'value_diff': round(float(peak_to_current_diff)),  # Convert to native Python float
        ...
        if change_since_date is not None:
            ...
            if not since_date_df.empty:
                ...
                results['change_since_date'] = {
                    'value_diff': float(since_date_diff),  # Convert to native Python float
